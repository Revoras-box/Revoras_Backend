...
export const sendVerificationCode = async (req, res) => {
  try {
    ...
    // Every signup (customer and business owner alike) creates a `users` row
    // now (report.md §2.0/§2.2) - was checking the dropped `barbers` table,
    // a leftover from before the Owner/Barber split was unified. Fixed as
    // part of Phase 2.3's registration rewrite (report.md Phase 2 plan).
    ...
    await storeOTP(email, emailOtp);
    await storeOTP(phone, phoneOtp);
