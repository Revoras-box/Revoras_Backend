...
- ✅ **Seed/dev coordinate backfill** — `db/seeds/dev/08_map_fixtures.js`: 12 active businesses across real Bengaluru neighbourhoods (dense, one city, so viewport/pan/"Search this area" are exercisable), plus the dev `test-barbershop` moved to Koramangala. **Verified end-to-end:** full-city bbox → 13, tight Koramangala box → 4, nearby+radius → distance-sorted. **Market confirmed: India.** Note: `06_dev_fixtures.js` does a destructive user-delete that now FK-conflicts with test bookings — reseeding it needs fixing (targeted update used for the dev DB).
- ✅ **Dev/demo seeds isolated from clones (2026-07-23)** — all fabricated fixtures (`05_dev_admin`, `06_dev_fixtures`, `07_jk_showcase`, `08_map_fixtures`) moved to `db/seeds/dev/`, run only via opt-in `npm run db:seed:dev` (refuses under `NODE_ENV=production`). Default `npm run db:seed` now seeds reference data only (roles/permissions/categories) — a fresh clone or production deploy no longer creates fake businesses/users. The fictional-Manhattan `/locations` frontend page (plus the other legacy fake-data marketing routes `/prelogin`, `/services`, `/barbers`, `/experience`) was **deleted** in the same pass.
